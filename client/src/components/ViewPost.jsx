          ...
          <article className="max-w-auto markdown">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-4 gap-8 mb-8">
              <div className="flex flex-col items-start gap-3 mb-3 w-full lg:col-span-3 order-1 lg:order-none">
                ...
                <div>{post.content && <ReactMarkdown>{post.content}</ReactMarkdown>}</div>
                <div className="rounded-2xl bg-stone-100 p-5 shadow-sm border border-gray-200 w-full mt-6 block lg:hidden">
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <div className="w-1/2">
                      ...
                      <div className="text-lg font-extrabold text-gray-900 leading-none">{post.author}</div>
                    </div>
                  </div>
                  <hr className="border-t-2 border-gray-200 mb-4" />
                  <p className="text-sm text-gray-600 mb-4">
                    I am a pet enthusiast and freelance writer who specializes in animal behavior and care. With a deep
                    love for cats, I enjoy sharing insights on feline companionship and wellness.
                  </p>
                  <p className="text-sm text-gray-600">
                    When I'm not writing, I spend time volunteering at my local animal shelter, helping cats find loving
                    homes.
                  </p>
                </div>
                      ...
                      items={[
                        {
                          label: (
                            <div
                              className={`flex items-center cursor-pointer ${likeDisabled ? "opacity-50 cursor-not-allowed" : ""} ${isLiked ? "text-red-500" : ""}`}
                              onClick={handleClick}
                            >
                              <CiFaceSmile />
                ...
                <div className="w-full rounded-3xl bg-stone-100 p-3 flex flex-col items-center gap-3 shadow-sm mt-4ex-wrap lg:hidden mb-10">
                  <button
                    className={`flex items-center border border-gray-300 rounded-full px-6 py-2 bg-white text-black text-base font-medium shadow-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-gray-300 ${likeDisabled ? "opacity-50 cursor-not-allowed" : ""} ${isLiked ? "text-red-500 border-red-300" : ""}`}
                    onClick={handleClick}
                ...
                <div className="flex flex-col w-full gap-3 ">
                  ...
                  <Textarea
                    ...
                    disabled={!isLoggedIn}
                    readOnly={!isLoggedIn}
                    className={!isLoggedIn ? "cursor-not-allowed opacity-50" : "cursor-text"}
                  />
                  <button
                    className={`text-white bg-gray-800 hover:bg-gray-900 focus:outline-none focus:ring-gray-300 font-medium rounded-full text-sm me-2 mb-2 dark:bg-gray-800 dark:hover:bg-[hsla(36,4%,44%,1)] dark:focus:ring-gray-700 dark:border-stone-400 px-[40px] py-[12px] border-1 w-fit lg:ml-auto ${!isLoggedIn ? "opacity-50 cursor-not-allowed" : "cursor-pointer"}`}
                    disabled={!isLoggedIn}
        ...
        {showDialog && (
          ...
          >
            ...
            >
              ...
              <h2 className="text-3xl font-bold text-gray-900 mb-8 mt-4">Create an account to continue</h2>
              ...
              <p className="text-gray-600">
                ...
                <button className="text-gray-900 underline hover:no-underline font-medium"><Link to="/login">Log in</Link></button>
              </p>
            </div>
          </div>
        )}
